...
class GUI:
    ...
    def setWindow(self):
        self.root.title("Group 3")
        width, height = 600, 400  # 500, 530
        self.root.geometry("%dx%d" % (width, height))

        # Display infor, title
        # tk.Label(self.root, text="Group 3", font=(self.font, 10), fg=self.textColor).place(
        #     x=250, y=3)
        # tk.Label(self.root, text="Multimedia Data Coding", font=(self.font, 10), fg=self.textColor).place(
        #     x=180, y=23)
        # tk.Label(self.root, text="Group 3", font=(self.font, 16, "bold"), fg=self.textColor).place(x=150, y=48)
    ...
    def drawUI(self):
        ...
        dirEntry = tk.Entry(self.root, textvariable="", font=(self.font, 12), width=38)
        dirEntry.pack()
        dirEntry.insert(0, "")
        ...
        btnOpenFile.place(x=self.xForEntry+360, y=105)

        tk.Label(self.root, text="JPEG COMPRESSION", font=(self.font, 16, "bold"), fg=self.textColor).place(x=160, y=30)
        ...
        btnProcess = tk.Button(self.root,  text = "Process", width = 50, height = 2, bg = self.buttonColor, fg = "#FFFFFF", command=lambda:self.actionProcess(".jpg"))
        btnProcess.place(x = self.xForEntry, y = 200)
    ...
    def actionDecompress(self, typeFile):
        ...
        fileToSave = filename + "_decode.bmp"
    ...
    def actionProcess(self, typeFile):        
        ...
        outputPath = filename + "_decode.bmp"
